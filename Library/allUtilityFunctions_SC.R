...
list.SCRANnorm <-function(physeq){
...
eval_res_list_repeated<-function(resi, 
                                 ...
                                 alpha_range = c(0, 0.001, seq(0.01, 0.25, 0.01), seq(0.3, 1, 0.1))) {
...
#################################
# Get P-values from An BPSC Fit #
#################################

get_pval_BPSC<-function(fit){
  mat<-as.matrix(fit[,3,1])
  rownames(mat)<- row.names(fit[,,1])
  colnames(mat)<- names(metadata)
...
#####################################
# Get P-values from An DEsingle Fit #
#####################################

get_pval_DEsingle<-function(fit){
  mat<-as.matrix(fit[,3,1])
  rownames(mat)<- row.names(fit[,,1])
  colnames(mat)<- names(metadata)
...
####################################
# Get P-values from An monocle Fit #
####################################

get_pval_monocle<-function(fit){
  mat<-as.matrix(fit[,3,1])
  rownames(mat)<- row.names(fit[,,1])
  colnames(mat)<- names(metadata)
...
#################################
# Get P-values from An scDD Fit #
#################################

get_pval_scDD<-function(fit){
  mat<-as.matrix(fit[,3,1])
  rownames(mat)<- row.names(fit[,,1])
  colnames(mat)<- names(metadata)
...
###################################
# Get P-values from An zingeR Fit #
###################################

get_pval_zingeR<-function(fit){
  mat<-as.matrix(fit$coefficients)
  rownames<-rownames(mat)
  colnames<-colnames(mat)
  n<-dim(fit$coefficients)[2]
...
#################################
  ...
  combiner.fun<-function(x){round(mean(as.numeric(x),na.rm=T),2)}
  qvals<-df$qval
  alphas<-df$alpha
  qvals<-qvals[!duplicated(qvals)] 
  alphas<-alphas[!duplicated(alphas)]
  all.combs<-expand.grid(qval=qvals,alpha=alphas)
  all.combs$alpha<-as.numeric(levels(all.combs$alpha))[all.combs$alpha]
  all.combs$qval<-as.character(levels(all.combs$qval))[all.combs$qval]
  
  #################################
  ### Running for all iteration ###
  #################################
  
  all_iterationsResults<-NULL
  for(i in 1:nrow(all.combs)){
    alpha<-all.combs[i,]$alpha
    qval<-all.combs[i,]$qval
    tmp<-df[df$qval==qval & df$alpha==alpha,]
    
    ##################################
    ### Applying combiner function ###
    ##################################
    
    Sensitivity<-combiner.fun(tmp$Sensitivity)
    Specificity<-combiner.fun(tmp$Specificity)
    FDR<-combiner.fun(tmp$FDR)
    FScore<-combiner.fun(tmp$FScore)
    MCC<-combiner.fun(tmp$MCC)
    AUC<-combiner.fun(tmp$AUC)
    pAUC<-combiner.fun(tmp$pAUC)
    conservArea<-combiner.fun(tmp$conservArea)
    liberalArea<-combiner.fun(tmp$liberalArea)
    totalArea<-combiner.fun(tmp$totalArea)
    time<-combiner.fun(tmp$time)
    alpha<-tmp$alpha[1]
    qval<-tmp$qval[1]
    methodName<-tmp$methodName[1]
    ZeroInflate<-tmp$ZeroInflate[1]
    RandomEffect<-tmp$RandomEffect[1]
    metadataType<-tmp$metadataType[1]
    nCells<-tmp$nCells[1]
    nPerSubject<-tmp$nPerSubject[1]
    nGenes<-tmp$nGenes[1]
    nMetadata<-tmp$nMetadata[1]
    effectSize<-tmp$effectSize[1]
    dropout.mid<-tmp$dropout.mid[1]
    pDE<-tmp$pDE[1]
    nIterations<-nrow(tmp)
    combinations<-paste(methodName,
                        ZeroInflate,
                        RandomEffect,
                        metadataType,
                        nCells,
    ...
    tmp1<-data.frame(combinations = combinations,
    ...
    all_iterationsResults<-data.frame(rbind(all_iterationsResults,tmp1))
  }
...
SingleCell_Simulator<-
  function(simulator = 'splatter', 
    ...
    SplatterParams <- splatter::newSplatParams()
    if (!is.null(template)) {
      SplatterParams<-splatter::splatEstimate(template)
    }
    ...
    for (i in seq_along(simparams)){
      ...
      SplatterParams <- splatter::setParam(SplatterParams, "nGenes", nGenes) 
      SplatterParams <- splatter::setParam(SplatterParams, "batchCells", nCells)
      SplatterParams <- splatter::setParam(SplatterParams, "de.facLoc", effectSize) 
      SplatterParams <- splatter::setParam(SplatterParams, "dropout.type", "experiment")
      SplatterParams <- splatter::setParam(SplatterParams, "dropout.type", ifelse(dropout=="1", 'experiment', 'none'))
      SplatterParams <- splatter::setParam(SplatterParams, "dropout.mid", 3)
      SplatterParams <- splatter::setParam(SplatterParams, "seed", i)
      sim <- splatter::splatSimulate(SplatterParams, group.prob=c(.5,.5), method="groups", verbose = FALSE)
